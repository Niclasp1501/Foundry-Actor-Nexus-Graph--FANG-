...
### Added
- **See who is editing what right now.** When someone else has a character open in the editor, the node gets a dashed ring in that person's Foundry colour for everyone else, and a label "Anna is editing" above it. In the context menu, "Edit" is locked for that node and for an open connection, with the name as the reason. Whoever opens it anyway is asked: "{user} has {name} open right now. If you both save, the last one wins." The banner for shared editing names, after each person, what they have open. None of this is stored: every client reports every three seconds, and whoever is silent for eight seconds disappears by itself. First building block from `KONZEPT-GEMEINSAMES-BEARBEITEN.md`.

- **A dragged node moves for everyone.** Whoever drags a node sends its position during the drag (at most every 80 milliseconds) and once on release. The others see the node move along, with a label "{user} is moving", and hand it back to their own physics on release. If the release never comes because the person dragging lost their connection, the node is released after two and a half seconds. As before, only the person dragging saves. Second building block.

- **Conflicts are shown, not decided.** When two people changed the same text field (note, role, label of a connection), the second only got a hint until now, and the first person's version was gone. Now a window opens with both versions, name and field included, and you choose what stays. For checkboxes, affiliations and positions "the last one wins" remains, which is right there. Building block 5.

- **A history with undo.** The new button in the bar (gamemaster only) shows who changed what and when: one entry per save, with the operations in plain words underneath ("Baron Valmont, role: mercenary → lord of the house", "Connection knows Elara / Garrek: removed"). Every entry has "Undo". It reverses the operations of the entry but leaves every field that has been changed again since, and says how many those were. The undo is an entry itself and can be undone again. Building block 6. With it every building block of the concept is built; what remains open is what only a table can show.

- **Saving without a gamemaster waits instead of being lost.** A player who saved while no gamemaster was online got a warning, and the change was gone. Now it stays in the window, the banner shows "Changes waiting for the gamemaster", and as soon as a gamemaster connects it goes out. The basis for the merge stays the one from the first waiting save, so the gamemaster sees everything that has changed since. Building block 4a.
...
### Changed
- **Saving sends operations instead of whole graphs.** Until now every save compared three complete graphs (the loaded state, your own state, the saved state) and merged them. Now FANG first works out the list of operations since loading ("set Baron Valmont's role to lord of the house", "delete connection Baron/Lyra") and applies that list to the saved state. The result is the same as before, proven in `tools/fang-ops-test.mjs` for every situation the merge tests cover. What the list can do in addition: a player sends the gamemaster a few operations instead of two whole graphs; a conflict names field, old and new value; every operation knows the value it replaced, so it can be reversed; and whoever writes keeps a log. Building block 3 from `KONZEPT-GEMEINSAMES-BEARBEITEN.md`. The old merge stays as a fallback for states that do not yet carry connection ids.

- **A history is recorded.** On every save the writing client files an entry: who, when, how many operations of which kind, and the operations themselves (up to 60 per entry, 300 entries in total). A change a player sent through the gamemaster is recorded under the player's name. The window for it and the undo come as building block 6.
