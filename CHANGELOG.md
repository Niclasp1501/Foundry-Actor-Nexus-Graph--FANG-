...
- **Ein gezogener Knoten bewegt sich bei allen.** Wer einen Knoten zieht, sendet die Position während des Ziehens (höchstens alle 80 Millisekunden) und einmal beim Loslassen. Die anderen sehen den Knoten mitwandern, mit Schild „{user} verschiebt", und geben ihn beim Loslassen an ihre eigene Physik zurück. Bleibt das Loslassen aus, weil der Ziehende die Verbindung verloren hat, wird der Knoten nach zweieinhalb Sekunden freigegeben. Gespeichert wird wie bisher erst beim Ziehenden. Zweiter Baustein.

- **Konflikte werden gezeigt, nicht entschieden.** Haben zwei Leute dasselbe Textfeld geändert (Notiz, Rolle, Bezeichnung einer Verbindung), bekam der Zweite bisher nur einen Hinweis, und die Fassung des Ersten war weg. Jetzt öffnet sich ein Fenster mit beiden Fassungen, Name und Feld dabei, und man wählt, was bleibt. Für Kästchen, Zugehörigkeiten und Positionen bleibt „der Letzte gewinnt", dort ist das richtig. Baustein 5.

- **Ein Verlauf mit Rückgängig.** Der neue Knopf in der Leiste (nur Spielleitung) zeigt, wer wann was geändert hat: je Speichern ein Eintrag, darunter die Vorgänge im Klartext („Baron Valmont, Rolle: Söldner → Hausherr", „Verbindung kennt Elara / Garrek: entfernt"). An jedem Eintrag steht „Rückgängig". Es nimmt die Vorgänge des Eintrags zurück, lässt aber jedes Feld stehen, das seitdem erneut geändert wurde, und sagt, wie viele das waren. Das Zurücknehmen ist selbst ein Eintrag und lässt sich wieder zurücknehmen. Baustein 6. Damit sind alle Bausteine des Konzepts gebaut; offen bleibt, was nur ein Tisch zeigen kann.

- **Speichern ohne Spielleitung wartet, statt zu verfallen.** Ein Spieler, der speichert, während keine Spielleitung online ist, bekam eine Warnung, und die Änderung war weg. Jetzt bleibt sie im Fenster liegen, das Banner zeigt „Änderungen warten auf die Spielleitung", und sobald eine Spielleitung verbindet, geht sie ab. Die Grundlage für den Abgleich bleibt dabei die vom ersten wartenden Speichern, damit die Spielleitung sieht, was sich seitdem alles geändert hat. Baustein 4a.
...
### Changed
- **Speichern schickt Vorgänge statt ganzer Graphen.** Bisher verglich jedes Speichern drei vollständige Graphen (geladener Stand, eigener Stand, gespeicherter Stand) und führte sie zusammen. Jetzt ermittelt FANG zuerst die Liste der Vorgänge seit dem Laden („setze bei Baron Valmont die Rolle auf Hausherr", „lösche Verbindung Baron/Lyra") und wendet diese Liste auf den gespeicherten Stand an. Das Ergebnis ist dasselbe wie vorher, bewiesen in `tools/fang-ops-test.mjs` für jede Situation, die die Abgleich-Tests abdecken. Was die Liste zusätzlich kann: Ein Spieler schickt der Spielleitung ein paar Vorgänge statt zwei ganzer Graphen; ein Konflikt nennt Feld, alten und neuen Wert; jeder Vorgang kennt den Wert, den er ersetzt hat, also lässt er sich umkehren; und wer schreibt, führt ein Protokoll. Baustein 3 aus `KONZEPT-GEMEINSAMES-BEARBEITEN.md`. Der alte Abgleich bleibt als Rückfall für Stände, die noch keine Verbindungs-Kennungen tragen.

- **Ein Verlauf entsteht.** Bei jedem Speichern legt der schreibende Client einen Eintrag ab: wer, wann, wie viele Vorgänge welcher Art, und die Vorgänge selbst (bis 60 je Eintrag, 300 Einträge insgesamt). Eine Änderung, die ein Spieler über die Spielleitung geschickt hat, steht unter dem Namen des Spielers. Das Fenster dazu und das Rückgängig kommen als Baustein 6.
