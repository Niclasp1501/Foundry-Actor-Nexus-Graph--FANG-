...
- **Collaborative Editing (Optional):** Instead of an exclusive lock, several people can work on the graph at once. Saving merges field by field against the stored state, so two people editing different things no longer overwrite each other. Physics drift is discarded on purpose; only positions someone actually dragged count as intent. Everyone sees who has which character open: a ring in that person's colour, their name above the token, and the menu says so before you open the same editor. Drags show live for everyone. If two people still change the same text, both versions are shown and you pick. A change log (GM only) lists who changed what, with undo per entry, and a player's save waits for a GM instead of being lost.
...
- **Gemeinsames Bearbeiten (optional):** Statt einer exklusiven Sperre können mehrere Personen gleichzeitig am Graphen arbeiten. Speichern führt die Änderungen feldweise mit dem gespeicherten Stand zusammen, sodass zwei Leute an verschiedenen Dingen sich nicht mehr gegenseitig überschreiben. Physik-Drift wird dabei bewusst verworfen; nur gezogene Positionen zählen als Absicht. Jeder sieht, wer gerade welchen Charakter offen hat: ein Ring in der Farbe dieser Person, ihr Name über dem Token, und das Menü sagt es, bevor man denselben Editor öffnet. Ziehen sehen alle live. Ändern zwei Leute trotzdem denselben Text, werden beide Fassungen gezeigt und man wählt. Ein Verlauf (nur Spielleitung) zeigt, wer was geändert hat, mit Rückgängig je Eintrag; und ein Speichern eines Spielers wartet auf die Spielleitung, statt zu verfallen.
