...
- [ ] **6.3 Real collaborative editing / Echtes gemeinsames Bearbeiten:** Decided 11.09.2026, concept in `KONZEPT-GEMEINSAMES-BEARBEITEN.md`. Decisions taken: soft lock per element that shows and blocks with an override; no direct writing for players (a player who could write the flag could delete the journal); the operations rewrite is wanted. All six blocks are built and on the beta channel: presence, live drags, waiting saves, operations, visible conflicts, change log with undo. What remains is what only a table can show: two or more people editing at once in a real session, on desktop and tablet, with and without a GM in the graph. Until that has happened this stays open. Every block ships to beta on its own / Entschieden am 11.09.2026, Konzept in `KONZEPT-GEMEINSAMES-BEARBEITEN.md`. Entscheidungen: weiche Sperre je Element, die anzeigt und blockiert, mit Uebersteuern; kein direktes Schreiben fuer Spieler (wer das Flag schreiben kann, kann das Journal loeschen); der Vorgangs-Umbau ist gewollt. Alle sechs Bausteine sind gebaut und auf dem Beta-Kanal: Anwesenheit, Ziehen live, wartende Speichervorgaenge, Vorgaenge, sichtbare Konflikte, Verlauf mit Rueckgaengig. Offen ist, was nur ein Tisch zeigen kann: zwei oder mehr Leute, die in einer echten Sitzung gleichzeitig bearbeiten, am Schreibtisch und am Tablet, mit und ohne Spielleitung im Graphen. Bis das passiert ist, bleibt der Punkt offen. Jeder Baustein geht einzeln auf Beta.
